...
class AIContextCraft:
    ...
    def __init__(self, config_path: str = None, source_dir: str = "."):
        ...
        self.processor.set_git_manager(self.git)
    ...
    def concat_files(self, output_file: str = None, 
        ...
        if to_clipboard:
            ...
            if clipboard.clipboard_available:
                if clipboard.copy_file_to_clipboard(output_file):
                    print(f"📋 Content copied to clipboard! ({self._format_size(output_file.stat().st_size)})")
                else:
                    print("⚠️  Failed to copy to clipboard")
            else:
                print(f"⚠️  Clipboard not available: {clipboard.get_clipboard_info()}")
    ...
    def _write_output(self, output_file: Path, result: Dict, tree_content: str, strip_comments: bool):
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            ...
            if self.git and self.git.is_git_repo:
                for line in self.git.get_info_for_header():
                    f.write(line + "\n")
